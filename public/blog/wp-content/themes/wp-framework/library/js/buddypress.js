...
var wpf_bp_ajax_request = null;

jq(document).ready( function() {
	/**** registration/register.php *******************************************************/
	if ( jq('div#blog-details').length && !jq('div#blog-details').hasClass('show') )
		jq('div#blog-details').toggle();

	jq( 'input#signup_with_blog' ).click( function() {
		jq('div#blog-details').fadeOut().toggle();
	});
	...
	wpf_bp_init_activity();

	/* Object filter and scope set. */
	var objects = [ 'members', 'groups', 'blogs', 'forums' ];
	wpf_bp_init_objects( objects );
	...
	jq('div.activity-type-tabs').click( function(event) {
		...
		wpf_bp_activity_request(scope, filter, target);
	...
	jq('#activity-filter-select select').change( function() {
		...
		wpf_bp_activity_request(scope, filter);
			...
			function(response) {
				...
				if ( 'fav' == type ) {
					if ( !jq('div.item-list-tabs li#activity-favorites').length )
						jq('div.item-list-tabs ul li#activity-mentions').before( '<li id="activity-favorites"><a href="#">' + WPF_BP.my_favs + ' (<span>0</span>)</a></li>');
					...
					if ( !Number( jq('div.item-list-tabs ul li#activity-favorites span').html() ) ) {
						if ( jq('div.item-list-tabs ul li#activity-favorites').hasClass('selected') )
							wpf_bp_activity_request( null, null );
	...
	jq('span.highlight span').click( function() {
		if ( !jq('div.help').length ) {
			jq(this).parent().after( '<div id="message" class="info help"><p>' + WPF_BP.mention_explain + '</p></div>' );
	...
	jq('div.dir-search').click( function(event) {
		...
		if ( target.attr('type') == 'submit' ) {
			...
			wpf_bp_filter_request( object, jq.cookie('bp-' + object + '-filter'), jq.cookie('bp-' + object + '-scope') , 'div.' + object, target.parent().children('label').children('input').val(), 1, jq.cookie('bp-' + object + '-extras') );
	...
	jq('div.item-list-tabs').click( function(event) {
		...
		if ( 'LI' == event.target.parentNode.nodeName && !target.hasClass('last') ) {
			...
			wpf_bp_filter_request( object, filter, scope, 'div.' + object, search_terms, 1, jq.cookie('bp-' + object + '-extras') );
	...
	jq('div#content').click( function(event) {
		...
		if ( target.parent().parent().hasClass('pagination') && !target.parent().parent().hasClass('no-ajax') ) {
			...
			wpf_bp_filter_request( object, jq.cookie('bp-' + object + '-filter'), jq.cookie('bp-' + object + '-scope'), 'div.' + object, search_terms, page_number, jq.cookie('bp-' + object + '-extras') );
	...
	jq('div#forum-directory-tags a').click( function() {
		wpf_bp_filter_request( 'forums', 'tags', jq.cookie('bp-forums-scope'), 'div.forums', jq(this).html().replace( /&nbsp;/g, '-' ), 1, jq.cookie('bp-forums-extras') );
		return false;
	});
		...
		function(response) {
			...
			if ( response[0] + response[1] == '-1' ) {
				...
				button.fadeOut( 100, function() {
					if ( jq(this).hasClass('accept') ) {
						jq(this).html( WPF_BP.accepted ).fadeIn(50);
						jq(this).addClass('accepted');
					} else {
						jq(this).html( WPF_BP.rejected ).fadeIn(50);
...
function wpf_bp_init_objects(objects) {
	jq(objects).each( function(i) {
...
function wpf_bp_filter_request( object, filter, scope, target, search_terms, page, extras ) {
	if ( 'activity' == object )
		return false;
	...
	if ( wpf_bp_ajax_request )
		wpf_bp_ajax_request.abort();

	wpf_bp_ajax_request = jq.post( ajaxurl, {
...
function wpf_bp_activity_request(scope, filter) {
	...
	comments_divs.each( function() {
		...
		comment_lis.each( function(i) {
			...
			if ( i < comment_lis.length - 5 ) {
				...
				if ( !i )
					jq(this).before( '<li class="show-all"><a href="#' + parent_li.attr('id') + '/show-all/" title="' + WPF_BP.show_all_comments + '">' + WPF_BP.show_all + ' ' + comment_count + ' ' + WPF_BP.comments + '</a></li>' );
			}
		});
