<?php
/**
 * BuddyPress Template: Forums Loop
 *
 * This template displays the BuddyPress forums loop.
 *
 * The querystring is set via AJAX in _inc/ajax.php - wpf_bp_object_filter().
 *
...
<?php if ( bp_has_forum_topics( bp_ajax_querystring( 'forums' ) ) ) : ?>
	...
	<table class="forum zebra">
		<thead>
			<tr>
				<th id="th-title"><?php _e( 'Topic Title', t() ); ?></th>
				<th id="th-poster"><?php _e( 'Latest Poster', t() ); ?></th>

				<?php if ( !bp_is_group_forum() ) : ?>
					<th id="th-group"><?php _e( 'Posted In Group', t() ); ?></th>
				<?php endif; ?>

				<th id="th-postcount"><?php _e( 'Posts', t() ); ?></th>
				<th id="th-freshness"><?php _e( 'Freshness', t() ); ?></th>
		...
		<tbody>
			...
			<tr class="<?php bp_the_topic_css_class(); ?>">
				<td class="td-title">
					<a class="topic-title" href="<?php bp_the_topic_permalink(); ?>" title="<?php bp_the_topic_title(); ?> - <?php _e( 'Permalink', t() ); ?>">
	...
	<div id="pag-bottom" class="pagination">
		...
		<div class="pag-count" id="topic-count-bottom">
			<?php bp_forum_pagination_count(); ?>
		</div><!-- #topic-count-bottom .pag-count -->

		<div class="pagination-links" id="topic-pag-bottom">
			<?php bp_forum_pagination(); ?>
		</div><!-- #topic-pag-bottom .pagination-links -->

	</div><!-- #pag-bottom .pagination -->

<?php else: ?>

	<?php wpf_message( __( 'Sorry, there were no forum topics found.', t() ), 'info' ); ?>
