...
class Hybrid_Providers_Google extends Hybrid_Provider_Model_OAuth2
{
	...
	*/
	function getUserProfile()
	{
		...
		// dd(json_decode(file_get_contents('/110880509059057751100?access_token='.$this->api->access_token)));
		// die();

		$this->user->profile->identifier    = (property_exists($response,'id'))?$response->id:"";
		$this->user->profile->age           = (property_exists($response,'ageRange'))?$response->ageRange->min:"";
		...
		$this->user->profile->displayName   = (property_exists($response,'name'))?$response->displayName:"";
		$this->user->profile->photoURL      = (property_exists($response,'image'))?$response->image->url:"";
		$this->user->profile->profileURL    = (property_exists($response,'url'))?$response->url:"";
		$this->user->profile->gender        = (property_exists($response,'gender'))?$response->gender:"";
		$this->user->profile->emailVerified = (property_exists($response,'verified'))?$response->verified:"";
		$this->user->profile->language      = (property_exists($response,'language'))?$response->language:"";

		$urls = "";
		if( property_exists($response,'urls') ) {
			if(is_array($response->emails)) {
				// response array of emails has properties: value, type
				$emails = [];
				foreach($response->emails as $data) {
					$emails[] = $data->value;
				}
				$email = join(',', $emails);
			}
		...
		$this->user->profile->webSiteURL = $urls;

		$region = "";
		if(property_exists($response,'placesLived')) {
			if(is_array($response->placesLived)) {
				foreach($response->placesLived as $place) {
					if(property_exists($place, 'primary')) {
						$region = $place->value;
						break;
					}
				}
